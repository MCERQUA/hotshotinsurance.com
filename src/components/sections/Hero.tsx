  ...
  return (
    <section className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      <div className="absolute inset-0 z-0">
        <img src="/images/hero-bg.jpg" alt="" className="w-full h-full object-cover object-center" loading="eager" aria-hidden="true" />
      ...
      <div className="container-wide relative z-10">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
          <motion.div className="lg:col-span-6" {...container}>
            <motion.div {...item}>
              <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-heading font-semibold uppercase tracking-wider bg-white/15 text-cream border border-white/25">
                <span className="h-1.5 w-1.5 rounded-full bg-gold animate-pulse" />
                {BRAND.regionPill}
              </span>
            </motion.div>

            <motion.h1 {...item} className="mt-5 font-heading font-extrabold text-white text-4xl sm:text-5xl lg:text-[3.5rem] xl:text-6xl leading-[1.05] tracking-tight">
              {COPY.hero.h1Lead}{" "}
              <span className="text-gold">{COPY.hero.h1Highlight}</span>.
            </motion.h1>

            <motion.p {...item} className="mt-6 text-lg md:text-xl text-cream/90 leading-relaxed max-w-xl">{COPY.hero.subcopy}</motion.p>
            ...
            <motion.div {...item} className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-cream/80">
              <span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-gold" />15-minute quotes</span>
              <span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-gold" />2-hour claims response</span>
              <span className="inline-flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-gold" />Licensed all 50 states</span>
          ...
          <motion.div className="lg:col-span-6 relative" initial={prefersReduced ? {} : { opacity: 0, scale: 0.96 }} animate={prefersReduced ? {} : { opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }}>
            <div className="relative">
              <div className="relative overflow-hidden rounded-t-[6rem] rounded-b-3xl shadow-warm-lg border-4 border-white/30">
