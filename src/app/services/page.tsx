...
import {
  Truck, PackageCheck, Wrench, Route, Container, HeartPulse,
  ShieldCheck, Umbrella, ArrowRight,
} from "lucide-react";

const ICONS = {
  Truck, PackageCheck, Wrench, Route, Container, HeartPulse,
  ShieldCheck, Umbrella,
} as const;
      ...
      <main>
        <section className="relative pt-32 pb-16 md:pt-40 md:pb-20 overflow-hidden">
          ...
          <div className="container-tight text-center relative z-10">
            <FadeIn>
              <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/15 text-cream text-xs font-heading font-bold uppercase tracking-[0.18em] border border-white/25">Coverage lines</span>
              <h1 className="mt-5 font-heading font-extrabold text-white text-4xl md:text-5xl lg:text-6xl leading-[1.05] tracking-tight">
                ...
                <span className="text-gold">{COPY.servicesPage.h1Highlight}</span>.
              </h1>
              <p className="mt-6 text-lg md:text-xl text-cream/90 leading-relaxed max-w-2xl mx-auto">{COPY.servicesPage.lead}</p>
            </FadeIn>
          </div>
        </section>
          ...
          <div className="container-wide">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {SERVICES.map((s, i) => {
                ...
                const featured = s.slug === "trucking-liability";
                return (
